...
function scrapeWebsite($url,$id) {
    ...
    if (curl_errno($ch)) {  
        global $pdo;      
        $pdo->query("UPDATE  `scraped_sanfoundry_links` set `status` =8 WHERE id=$id");
        header("Location: http://localhost/it/scraper/sanfoundry/githtml.php");
...
$result = $pdo->query("select * from `scraped_sanfoundry_links` where `status` =0  order by id asc LIMIT 1");
...
foreach($rows as $row) {
    $id=$row['id'];
    $url=trim($row['url']);
    $pdo->query("UPDATE  `scraped_sanfoundry_links` set `status` =2 WHERE id=$id");
}
echo '<h1>'.$id.':::'.$url.'</h1>';

// Main logic
//$website_url = "https://www.sanfoundry.com/"; // Replace with the target URL
...
sleep(5);
//header("Location: http://localhost/it/scraper/sanfoundry/githtml.php?".$id);
ob_end_flush();
?>
<script>
    window.location.href = "http://localhost/it/scraper/sanfoundry/githtml.php?id=1";
</script>
