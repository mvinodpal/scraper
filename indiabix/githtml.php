...
function scrapeWebsite($url,$id) {
    ...
    if (curl_errno($ch)) {  
        global $pdo;      
        $pdo->query("UPDATE  `scraped_indiabix_links` set `status` =8 WHERE id=$id");
        header("Location: http://localhost/it/scraper/indiabix/githtml.php");
...
function extractLinks($html, $base_url) {
    ...
    foreach ($links as $link) {
        ...
        if (!empty($absoluteUrl)) {
            if (str_contains(strtolower($absoluteUrl), strtolower('indiabix'))) {
...
foreach($rows as $row) {
    $id=$row['id'];
    $url=trim($row['url']);
    $pdo->query("UPDATE  `scraped_indiabix_links` set `status` =2 WHERE id=$id");
}
echo '<h1>'.$id.':::'.$url.'</h1>';
...
sleep(10);
//header("Location: http://localhost/it/scraper/indiabix/githtml.php?".$id);
ob_end_flush();
?>
<script>
    window.location.href = "http://localhost/it/scraper/indiabix/githtml.php?id=1";
</script>
